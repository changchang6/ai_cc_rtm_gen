 ...
 **关键**：生成的RTM文件与example中示例文件比较（内容具体和可操作性），不足之处改进
...
- `examples/`: 参考示例文件
- `reference/checker_patterns.md`: Checker模式参考（按类别分类的checker模板）
- `reference/testcase_patterns.md`: Testcase模式参考（按类别分类的testcase模板）
- `reference/lrs_format_guide.md`: LRS文档格式指南（关键信息提取方法）
